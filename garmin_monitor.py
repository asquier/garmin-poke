...
def check_for_updates(client: Garmin, state: Dict[str, Any]) -> List[Update]:
    """Check for new data and return messages with state changes to apply on success"""
    updates = []
    ...
    try:
        ...
        if activities:
            ...
            if latest_activity_id != state.get('last_activity_id'):
                updates.append((
                    format_activity(latest_activity),
                    'last_activity_id',
                    latest_activity_id,
                ))
...
def main():
    ...
    try:
        ...
        updates = check_for_updates(client, state)

        if updates:
            log(f"Found {len(updates)} update(s)")
            state_changed = False

            # Apply each state change only after Poke accepts the message.
            for message, state_key, state_value in updates:
                if send_to_poke(message, test_mode=args.test_mode):
